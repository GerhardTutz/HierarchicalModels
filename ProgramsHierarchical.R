  ...
  if(length(preddisp) ==0){
    ...
    if(tests=="LR") {
      ...
      for(v in 1:varnum){
        #C<-matrix(0,1,dbet)
        equal<-pred[v]
        d<-MakeDesignEqual(dat,k,nameresp,pred,preda,equal)
...
OrdinalHierNeutral<- function(dat,k,nameresp,predn,prednonn,preda,preddis,tests,preddisp, cum){
  ...
  hierred<-OrdinalHierarchical(datred,k-1,nameresp="red",prednonn,preda,preddis,tests ="no",preddisp=NULL)
  if(all(prednonn==preda) & all(prednonn==preddis) )hierred<-OrdinalHierarchical(datred,k-1,nameresp="red",prednonn,preda,preddis,tests = tests,preddisp=NULL)
...
MakeDesignEqual <- function(dat,k,nameresp,pred,preda,equal){  
