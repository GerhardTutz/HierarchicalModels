...
#####################
OrdinalHierarchical<- function(dat,k,nameresp,pred,preda,preddis,tests,preddisp){
  ...
  ##  tests="yes" only if pred=preda=preddis
  ...
  #fits <- optim(beta, LoglikInt, gr = NULL,d$respm,d$ngr,d$X,d$indagr,method = "BFGS",lower = -Inf, upper = Inf,control = list(), hessian = TRUE)
  #fits
  ...
  #paste("resp", "~", paste(predictors, collapse = " + "))
  ...
  thr<- rep(0,knew-1)
  numpar<-length(c(thr,thr,rep(.0,length(preddis)),rep(.0,length(preda)) ))
  C<-matrix(0, length(preddisp),numpar)
  
  for (i in 1:length(preddisp)){
    for (ii in 1:length(preddis)) if(preddis[ii]==pred[i])C[i,2*(knew-1)+ii]<-1
    for (ii in 1:length(preda)) if(preda[ii]==pred[i])C[i,2*(knew-1)+length(preddis)+ii]<-1  ###!!
    }
  
  fitsp <- optim(fits$par, LoglikIntPen, gr = scoreIntPen,d$respm,d$ngr,d$X,d$indagr,C, lambda=1000000,method = "BFGS",lower = -Inf, upper = Inf,control = list(), hessian = FALSE)
  fitsp$par
  #parmdisp<- cbind(fitsp$par,std,z,pval)
  parmdisp<-as.data.frame(fitsp$par)
  if(length(preddis)*length(preda)>0)row.names(parmdisp)<-c(seq(1,2*(knew-1),1),paste("disagree",preddis),paste("agree",preda))
  if(length(preddis)==0)row.names(parmdisp)<-c(seq(1,2*(knew-1),1),paste("agree",preda))
  if(length(preda)==0)row.names(parmdisp)<-c(seq(1,2*(knew-1),1),paste("disagree",preddis))
  parmdisp
...
scoreInt <- function (beta,respm,ngr,X,indagr){
  ...
  for (i in 1:n){
    ...
    cumprob[i,1]<- prob[i,1]
    if(k  >=3){
      for (j in 2:(k-1)) {prob[i,j]<-exp(etam[j,]%*%beta)/(1+exp(etam[j,]%*%beta)) -
      exp(etam[j-1,]%*%beta)/(1+exp(etam[j-1,]%*%beta))
      cumprob[i,j]<-exp(etam[j,]%*%beta)/(1+exp(etam[j,]%*%beta))}
              }
...
OrdinalHierNeutral<- function(dat,k,nameresp,predn,prednonn,preda,preddis,tests="yes",preddisp, cum){
  ...
  hierred<-OrdinalHierarchical(datred,k-1,nameresp="red",prednonn,preda,preddis,tests = "nyes",preddisp=NULL)
  if(all(prednonn==preda) & all(prednonn==preddis) )hierred<-OrdinalHierarchical(datred,k-1,nameresp="red",prednonn,preda,preddis,tests = "yes",preddisp=NULL)
